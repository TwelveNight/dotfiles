...
hl.bind(
	"CTRL + SUPER + Slash",
	hl.dsp.exec_cmd("xdg-open ~/.config/quickshell/ii/config.json"),
...
hl.bind("CTRL + ALT + L", hl.dsp.exec_cmd("playerctl next"), { locked = true, description = "Media: Next track" })
hl.bind("CTRL + ALT + Right", hl.dsp.exec_cmd("playerctl next"), { locked = true, description = "Media: Next track" })
hl.bind(
	"CTRL + ALT + H",
...
hl.bind(
	"CTRL + code:10",
	hl.dsp.exec_cmd(
		"qs -c $qsConfig ipc call TEST_ALIVE ping || pidof slurp || hyprshot --freeze --clipboard-only --mode region --silent"
	)
)
hl.bind(
	"CTRL + ALT + mouse:272",
...
hl.bind(
	"CTRL + ALT + mouse:273",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/screenshot.sh s"),
...
hl.bind(
	"CTRL + SHIFT + Z",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/1password.sh"),
	{ description = "Toggle 1Password" }
)
hl.bind(
	"CTRL + SHIFT + SPACE",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/1password-quick-access.sh"),
...
hl.bind(
	...
	{ description = "File browser (rofi)" }
)

-- XWayland file manager for dragging files to QQ/WeChat
hl.bind(
	"SUPER + ALT + E",
	hl.dsp.exec_cmd("env QT_QPA_PLATFORM=xcb pcmanfm-qt --profile xwayland --new-window"),
...
hl.bind("SUPER + SHIFT + ALT + P", function()
	hl.dispatch(
		hl.dsp.exec_cmd(
			"notify-send 'Entered clean submap' 'Keybinds disabled. Hit Super+Shift+Alt+P to escape' -a 'Hyprland'"
		)
	)
