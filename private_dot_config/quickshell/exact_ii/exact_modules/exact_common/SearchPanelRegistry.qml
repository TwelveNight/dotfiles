...
Singleton {
    ...
    // Mirrors Ai.qml's `enabled` (`aiPolicy !== 0`) straight from Config:
    // reading Ai.enabled from the descriptors below would construct the whole
    // Ai singleton while this registry is being created, for what is a pure
    // policy lookup. Keep in sync with services/Ai.qml.
    readonly property bool aiPolicyEnabled: Number(Config.options?.policies?.ai ?? 1) !== 0
    ...
    readonly property var panels: [
        ...
        { id: "ai", source: "AiChatPanel.qml", prefixKey: "ai", keywords: ["ai", "chat", "assistant"], label: qsTr("AI Chat"), icon: "auto_awesome", searchIcon: "auto_awesome", searchShape: "Clover4Leaf", searchRotationStep: 90, width: () => 720, enabled: () => root.aiPolicyEnabled, accent: false, focusMode: "input", queryProperty: "" },
