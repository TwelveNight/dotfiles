...
Singleton {
    ...
    readonly property bool currentModelUsesCodex: models[currentModelId]?.api_format === "codex"
    readonly property string currentToolDisplay: currentModelUsesCodex ? "native" : currentTool
    ...
    property AiMessageData codexResponseMessage
    property bool codexNeedsReplay: false
    ...
    property var toolDescriptions: {
        ...
        "native": Translation.tr("Codex's built-in tools, restricted to a read-only sandbox")
    }
    ...
    // - api_format: The API format of the model. Can be "openai", "gemini", "mistral", or "codex". Default is "openai".
    // - extraParams: Extra parameters to be passed to the model. This is a JSON object.
    property var models: Config.options.policies.ai === 2 ? {} : {
        "codex-current": aiModelComponent.createObject(this, {
            "name": "Codex (current model)",
            "icon": "openai-symbolic",
            "description": Translation.tr("Online via local Codex | Uses your existing Codex login and current model | Read-only sandbox"),
            "homepage": "https://developers.openai.com/codex/app-server/",
            "endpoint": "localhost://codex-app-server",
            "model": "",
            "requires_key": false,
            "api_format": "codex",
        }),
    ...
    function setModel(modelId, feedback = true, setPersistentState = true) {
        ...
        if (modelList.indexOf(modelId) !== -1) {
            ...
            const previousApiFormat = models[currentModelId]?.api_format;
            ...
            if (model.api_format === "codex" && previousApiFormat !== "codex") {
                codexClient.resetThread();
                root.codexNeedsReplay = root.messageIDs.length > 0;
            } else if (model.api_format !== "codex" && previousApiFormat === "codex") {
                codexClient.resetThread();
            }
    ...
    function setTool(tool) {
        if (root.currentModelUsesCodex) {
            if (tool === "native")
                return true;
            root.addMessage(Translation.tr("Codex uses its native tools in a read-only sandbox."), root.interfaceRole);
            return false;
        }
    ...
    function clearMessages() {
        ...
        codexClient.resetThread();
        root.codexNeedsReplay = false;
    }

    function codexReplayPrompt() {
        const transcript = root.messageIDs.map(id => root.messageByID[id])
            .filter(message => message.role === "user" || message.role === "assistant")
            .map(message => `${message.role === "user" ? "User" : "Assistant"}:\n${message.rawContent}`)
            .join("\n\n---\n\n");
        return "Continue the following sidebar conversation. Respond to the final user message only; do not repeat the transcript.\n\n" + transcript;
    }

    function finishCodexResponse() {
        if (!root.codexResponseMessage || root.codexResponseMessage.done)
            return;
        root.codexResponseMessage.thinking = false;
        root.codexResponseMessage.done = true;
        root.saveChat("lastSession");
        root.responseFinished();
    }

    function makeCodexRequest(latestMessage, forceReplay = false) {
        const shouldReplay = forceReplay || root.codexNeedsReplay;
        const prompt = shouldReplay ? root.codexReplayPrompt() : latestMessage;

        root.codexResponseMessage = root.aiMessageComponent.createObject(root, {
            "role": "assistant",
        ...
        const id = idForMessage(root.codexResponseMessage);
        root.messageIDs = [...root.messageIDs, id];
        root.messageByID[id] = root.codexResponseMessage;
        root.codexNeedsReplay = false;
        codexClient.startTurn(prompt, root.systemPrompt);
    }
    ...
    Connections {
        ...
        function onDelta(text) {
            if (!root.codexResponseMessage)
                return;
            root.codexResponseMessage.thinking = false;
            root.codexResponseMessage.content += text;
            root.codexResponseMessage.rawContent += text;
        }

        function onFinished() {
            root.finishCodexResponse();
        }

        function onFailed(message) {
            if (root.codexResponseMessage) {
                const errorText = `**Codex error**: ${message}`;
                root.codexResponseMessage.content += errorText;
                root.codexResponseMessage.rawContent += errorText;
            }
            root.codexNeedsReplay = true;
            root.finishCodexResponse();
        }

        function onTokenUsage(input, output, total) {
            root.tokenCount.input = input;
            root.tokenCount.output = output;
            root.tokenCount.total = total;
        }

        function onModelResolved(model) {
            const codexModel = root.models["codex-current"];
            if (codexModel)
                codexModel.name = `Codex · ${model}`;
        }
    ...
    function loadChat(chatName) {
        try {
            ...
            root.codexNeedsReplay = saveData.length > 0;
        } catch (e) {
